  ...
  return (
    <div
      ...
      className={`group flex items-center space-x-3 p-4 rounded-xl cursor-pointer transition-all  duration-200 border-2 ${
        isSelected 
          ? "bg-purple-700 border-primary-200 shadow-sm" 
          : "bg-gray-700 border-transparent  hover:border-primary-100 hover:shadow-sm"
      }`}
    >
      ...
      <div className="flex-1">
        <span className={`font-semibold transition-colors ${
          isSelected ? "text-primary-900" : "text-gray-100"
        }`}>
          Global Chat
        </span>
        <p className={`text-sm transition-colors ${
          isSelected ? "text-primary-600" : "text-gray-100"
        }`}>
          Chat with everyone
        </p>
      </div>
