...
export const UserSidebar: React.FC<UserSidebarProps> = ({
  ...
  const [modalUser, setModalUser] = useState<User | null>(null); // 🔥 track user for modal
  ...
  return (
    <>
        ...
        <div className="p-6 border-b border-gray-200 space-y-4 bg-gradient-to-r from-primary-50 to-blue-50">
          ...
          {currentUser && (
            <div className="flex items-center space-x-3 p-4 bg-white/80 backdrop-blur-sm rounded-xl shadow-sm border border-primary-100">
              ...
              <div onClick={() => setModalUser(currentUser)} className="cursor-pointer">
                <UserAvater user={currentUser} size="w-12 h-12" />
              </div>
          ...
          <div className="flex-1 overflow-y-auto p-4">
            <div className="space-y-1">
              <h3 className="text-sm font-semibold text-gray-600 mb-4 flex items-center space-x-2">
                ...
                <span>
                  Online Users (
                  {users.filter((u) => u.address !== currentUser?.address).length})
                </span>
              </h3>
                    ...
                    <div
                      ...
                      className={`group flex items-center space-x-3 p-3 rounded-xl cursor-pointer transition-all duration-200 ${
                        selectedUser?.address === user.address
                          ? "bg-gray-800 border-2 border-primary-200 shadow-sm"
                          : "hover:bg-gray-50 border-2 border-transparent hover:border-gray-100"
                      }`}
                    >
                      {/* 👇 avatar opens modal */}
                      <div onClick={() => setModalUser(user)} className="cursor-pointer">
                        <UserAvater user={user} />
                      </div>
                      {/* 👇 name/address still selects chat */}
                      <div
      ...
      {/* 🔥 Modal */}
      <UserModal user={modalUser} isOpen={!!modalUser} onClose={() => setModalUser(null)} />
    </>
  );
};
